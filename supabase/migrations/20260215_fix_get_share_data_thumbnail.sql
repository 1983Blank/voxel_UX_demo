-- Migration: Fix get_share_data function to get thumbnail_url from screens table
-- Date: 2026-02-04
-- Fixes: column vs.thumbnail_url does not exist error

-- Drop and recreate with correct join to screens table
...
BEGIN
  ...
  IF v_share.share_wireframes THEN
    ...
    SELECT
      ...
      sc.thumbnail_url  -- Get from screens table, not vibe_sessions
    FROM vibe_sessions vs
    LEFT JOIN screens sc ON sc.id = vs.screen_id  -- Join with screens to get thumbnail
