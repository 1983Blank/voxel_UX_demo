-- Migration: Fix get_share_data function - vibe_variants has 'screenshot_url' not 'thumbnail_url'
-- Date: 2026-02-04
-- Fixes: column vv.thumbnail_url does not exist error
...
BEGIN
  ...
  IF v_share.share_wireframes THEN
    ...
    SELECT
      ...
      sc.thumbnail AS thumbnail_url  -- screens table has 'thumbnail'
  ...
  ELSE
    ...
    SELECT
      ...
      vp.description,
      vs.name AS screen_name,
      COALESCE(vv.screenshot_url, sc.thumbnail) AS thumbnail_url  -- vibe_variants has 'screenshot_url', screens has 'thumbnail'
    FROM vibe_sessions vs
    LEFT JOIN screens sc ON sc.id = vs.screen_id
